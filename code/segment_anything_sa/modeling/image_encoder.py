...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        """
        Args:
            adapter_config (dict): Configuration setting for adapters.
        ...
        # SAM-Adapter (task specific tune)
        self.tune_out = self.adapter_config['task_specific_tune_out']
        ...
        self.task_adapter_up = nn.Linear(self.task_adapter_hidden_dim, embed_dim)
        
        # unshared down-projection layer 
        self.task_adapter_blocks = nn.ModuleList()
        for i in range(depth+1):
            adpt = Adapter(
                in_dim=self.tune_out,
                hidden_dim=self.task_adapter_hidden_dim,
                act_layer=self.adapter_config['task_specific_adapter_act_layer'],
                skip_connection=self.adapter_config['task_specific_adapter_skip_connection'],
                # shared up projection adapter 
                up_projection=self.task_adapter_up
            )
            self.task_adapter_blocks.append(adpt)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hfc = self.HFC_tune(hfc)
        
        task_specific_inform = patch_embed_tune + hfc 
        task_specific_inform = [adapter(task_specific_inform) for adapter in self.task_adapter_blocks]
        
        x = x + task_specific_inform[0]

        for i, blk in enumerate(self.blocks):
            x = blk(x)
            task_inform = task_specific_inform[i+1]
            x = x + task_inform
    ...
    def _extract_freq_components(self,
        ...
        high_fft = torch.fft.fftshift(torch.fft.fft2(x, norm="forward"))
        
        # High frequency components
        high_fft = high_fft * (1 - mask)
        high_fr, high_fi = high_fft.real, high_fft.imag
        
        hfc = torch.fft.ifftshift(torch.complex(high_fr, high_fi))
        hfc = torch.fft.ifft2(hfc, norm="forward").real
        hfc = torch.abs(hfc)
